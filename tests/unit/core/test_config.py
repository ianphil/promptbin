...
class TestPromptBinConfig:
    ...
    def test_validation_invalid_port_range(self):
        ...
        with pytest.raises(ServiceRegistrationError, match="Flask port 80 must be between 1024-65535"):
            PromptBinConfig(flask_port=80)
        
        # Test port above 65535  
        with pytest.raises(ServiceRegistrationError, match="Flask port 70000 must be between 1024-65535"):
            PromptBinConfig(flask_port=70000)

    def test_validation_invalid_log_level(self):
        """Test validation fails for invalid log levels."""
        with pytest.raises(ServiceRegistrationError, match="Log level 'INVALID' must be one of"):
    ...
    def test_validation_invalid_devtunnel_log_level(self):
        ...
        with pytest.raises(ServiceRegistrationError, match="Devtunnel log level 'INVALID' must be one of"):
    ...
    def test_validation_invalid_rate_limit(self):
        ...
        with pytest.raises(ServiceRegistrationError, match="Devtunnel rate limit 0 must be >= 1"):
    ...
    def test_validation_invalid_rate_window(self):
        ...
        with pytest.raises(ServiceRegistrationError, match="Devtunnel rate window 0 must be >= 1 minute"):
    ...
    def test_validation_invalid_timeouts(self):
        ...
        with pytest.raises(ServiceRegistrationError, match="Health check interval 1 must be >= 5 seconds"):
            PromptBinConfig(health_check_interval=1)
        
        with pytest.raises(ServiceRegistrationError, match="Shutdown timeout 0 must be >= 1 second"):
    ...
    def test_validation_nonexistent_parent_directory(self):
        ...
        with TemporaryDirectory() as temp_dir:
            ...
            with pytest.raises(ServiceRegistrationError, match="Parent directory of data_dir"):
    ...
    def test_to_dict_contains_all_fields(self):
        ...
        expected_fields = {
            "flask_host", "flask_port", "secret_key", "data_dir", "log_level",
            "devtunnel_enabled", "devtunnel_auto_start", "devtunnel_rate_limit",
            "devtunnel_rate_window", "devtunnel_log_level", "health_check_interval",
            "shutdown_timeout"
        }
